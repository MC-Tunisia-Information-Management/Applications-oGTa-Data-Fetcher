function getOpportunityApplications_oGTa() {
  var graphQLQuery = `
  ...
  var sheet = spreadsheet.getSheetByName("🏦 applications | oGTa");
  ...
  var data = applicationData.map(function (application) {
    return [
      ...
      application.opportunity.sub_product
  ...
  sheet.getRange("B7:P" + (data.length + 6)).setValues(data); // Adjusted range
}
